...
import { formatDate, getTime } from '../../helpers/date';
...
const SessionsList = () => {
    ...
    useEffect(() => {
        const getSessions = async () => {
            const req = await fetch(`${BACKENDADDRESS}cineticket/sessions`);
            const json = await req.json();

            if(sessions.length === 0) {
                json.data.map((sess: SessionType) => {
                    if(user.nivel === 'admin') {
                        ...
                        <TableBody>
                            ...
                            .map((sess, index) => (
                                ...
                                >
                                    ...
                                    <Tooltip title={getTime((new Date(sess.DATA)))} placement='top'>
                                        <TableCell>
                                            {`${formatDate((new Date(sess.DATA)))}`}
                                        </TableCell>
                                    </Tooltip>
                                    <TableCell>
                                        {sess.PRECO}
